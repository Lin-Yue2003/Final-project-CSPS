...
void data_initialize(struct item **head,const char *data_name){
    ...
    *head = NULL;
    while (fgets(line, MAX_LINE_LEN, fp)){
        ...
        token = strtok(NULL,",");
        new_item->preserve_days = atof(token);
        token = strtok(NULL,",");
        new_item->goods_num = atoi(token);// need to follow the goods list
        new_item->next = NULL;  
        new_item->oldest_goods = NULL;
        new_item->vec[0] = 0; //price
        new_item->vec[1] = 0; //calories
        for (int i = 2; i < MAX_VECTOR_NUM;i++){
            token = strtok(NULL,",");
            if (token == NULL){
                new_item->vec[i] = 0;
            }else {
    ...
    *head = NULL;
    while (fgets(line, MAX_LINE_LEN, fp)) {
        struct goods* new_item = malloc(sizeof(struct goods));
        if (new_item == NULL) printf("ERROR in LINE %d\n", __LINE__);
        char* token = strtok(line, ",");
        strcpy(new_item->name, token);
        token = strtok(NULL, ",");
        ...
        new_item->preserve_days = atof(token);
        token = strtok(NULL, ",");
        new_item->goods_num = atoi(token) + 1;  // need to follow the goods list
        new_item->next = NULL;
        new_item->prev = NULL;
        new_item->vec[0] = 0;  // price
        new_item->vec[1] = 0;  // calories
    ...
    // Add to list
    radd_to_item_list(head, new_item);
...
void radd_to_item_list(struct goods** head, struct goods* new_item) {
    ...
    if (*head == NULL) {
        *head = new_item;
        ...
        struct goods* last_item = *head;
        while (last_item->next != NULL) {
            last_item = last_item->next;
        }
        last_item->next = new_item;
        new_item->prev = last_item;  // 设置新节点的前一个节点指针
    }
}
...
void print_item_list(struct item *head){
    ...
    while(current != NULL){
        if(i%2==0){
            printf(Black_word_White_back"%2d. 名稱:%-s\t,價格:%d  熱量:%d  保存天數:%1f  尚餘存貨:%d"FINISH, 
                    i++, current->name, current->price, current->calories,
                    current->preserve_days, current->goods_num);
                    printf("\n");
        }else
        printf("%2d. 名稱:%-s\t,價格:%d  熱量:%d  保存天數:%1f  尚餘存貨:%d\n", 
                    i++, current->name, current->price, current->calories,
                    current->preserve_days, current->goods_num);
...
void data_update(struct item *head ,const char *data_name){
    ...
    while (p != NULL){
        ...
        fputs(temp, fp);
        fputc(',', fp);
        gcvt(p->preserve_days,6,temp);
        //itoa((int)(p->vec[2] * 100),temp);
        fputs(temp, fp);
        fputc(',',fp);
        itoa((int)p->goods_num,temp);
        fputs(temp,fp);
        for(int j = 2; j < MAX_VECTOR_NUM; j++){
            fputc(',', fp);
...
void goods_num_random(struct item *category[CATEGORY_NUM]){
    for (int i = 0; i < CATEGORY_NUM; i++){
        ...
        while(current != NULL){
            current->goods_num = current->goods_num - ((current->goods_num == 0)? 0:(rand() % current->goods_num)) + (rand() % 2);
            current = current->next;
        }
    }
}